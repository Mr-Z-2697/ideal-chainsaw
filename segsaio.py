        ...
        while job:
            for _i,_x in enumerate(penabled):
                if _x.poll()==None:
                    ...
                    # cmd=f'title piece {lastkf} to {_n+end} of {frames} (roughly {percentage}%) gops: {_g} & vspipe -c y4m "{lastkf}.vpy" - | vvencffapp -i - --y4m -ip 0 -rs 1024 -dr idr_no_radl --POC0IDR 1 --preset medium -t 16 --WaveFrontSynchro 1 --CTUSize 64 --IFP 1 --CIIP 3 --SAO 0 -qpa 1 -q 28 -b "{lastkf}.tmp.{extension}" && del "{lastkf}.vpy" && move/Y "{lastkf}.tmp.{extension}" "{lastkf}.{extension}"'
