        ...
        while job:
            for _i,_x in enumerate(penabled):
                if _x.poll()==None:
                    ...
                    # cmd=f'title piece {lastkf} to {_n+end} of {frames} (roughly {lastkf/frames*100}%) gops: {_g} & vspipe -c y4m "{lastkf}.vpy" - | vvencffapp -i - --y4m -ip 0 -rs 1024 -dr idr --preset medium -t 16 --WaveFrontSynchro 1 --CTUSize 64 --IFP 1 --CIIP 3 --SAO 0 -qpa 1 -q 28 -b "{lastkf}.tmp.{extension}" && del "{lastkf}.vpy" && move/Y "{lastkf}.tmp.{extension}" "{lastkf}.{extension}"'
                    # cmd=f'title piece {lastkf} to {_n+end} of {frames} (roughly {lastkf/frames*100}%) gops: {_g} & vspipe -c y4m "{lastkf}.vpy" - | uvg266-10 -i - --input-file-format y4m --input-bitdepth 10 --period 0 --preset veryslow --no-dep-quant --sao off --no-cclm --rd 3 --mv-rdo --qp 28 -o "{lastkf}.tmp.{extension}" && del "{lastkf}.vpy" && move/Y "{lastkf}.tmp.{extension}" "{lastkf}.{extension}"'
